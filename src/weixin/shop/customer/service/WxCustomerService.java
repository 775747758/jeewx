...
public class WxCustomerService {
	...
	public ShopCustomerEntity getCustomerWithId(String id,String fromUserName,String toUserName) {
		WeixinAccountEntity account = weixinAccountService.getEntity(WeixinAccountEntity.class,fromUserName);
		ShopCustomerEntity customer = shopCustomerService.get(ShopCustomerEntity.class, id);
	 ...
	 * @return
	 */
	public ShopCustomerEntity getCustomerWithCodeAndWeixinAccount(String code,WeixinAccountEntity account) {
		//防止null
		ShopCustomerEntity customer = new ShopCustomerEntity();
		if(StringUtils.isNotBlank(code)){
			//根据code和微信账户信息获取当前访问公众平台客户信息
			User user=WeChatUtil.getUserFromOauth2(code,account);
			customer=getCustomerWithfromUserNameAndtoUserName(user.getOpenid(), account.getWeixin_accountid());
			...
			shopCustomerService.doUpdateSql(customer);
		}
		return customer;
	}
	...
	public void initCustomer(String toUserName, String fromUserName){
		...
		if(getCustomerWithfromUserNameAndtoUserName(fromUserName, toUserName)==null) {
			...
			shopCustomerService.save(customer);
		}
	}
	 ...
	 * @return
	 */
	public ShopCustomerEntity getCustomerWithfromUserNameAndtoUserName(String fromUserName,String toUserName) {
		CriteriaQuery cq = new CriteriaQuery(ShopCustomerEntity.class);
		cq.eq("openid",fromUserName);
		cq.eq("weixinAccountid",toUserName);//原始ID
		cq.add();
		List<ShopCustomerEntity> list=shopCustomerService.getListByCriteriaQuery(cq, false);
